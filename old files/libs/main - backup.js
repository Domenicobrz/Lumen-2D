...
function init() {
    ...
    let testedge = new StraightEdge(vec2.fromValues(-4, -5), vec2.fromValues(-4, 5));
    let testedge2 = new StraightEdge(vec2.fromValues(-1, -2), vec2.fromValues(-1, 2));
    ...
    var s = 2;
    scene.add(emissionPoint, { type: "emitter", color:  [19*s, 5*s, 2 *s] });
...
function renderSample() {
    ...
    photonFired++;
    console.log("sample number: " + photonFired);
    ...
    for (var i = 0; i < canvasSize * canvasSize * 4; i += 4)
    {
        ...
        let r = pixelBuffer[y][x].r / (photonFired * 100);
        let g = pixelBuffer[y][x].g / (photonFired * 100);
        let b = pixelBuffer[y][x].b / (photonFired * 100);
...
function colorPhoton(ray, t, WORLD_SIZE, emitterColor, contribution, worldAttenuation) {
    ...
    for(let i = 0; i < SAMPLES; i++) {
        ...
        if(previousPixel[0] == px && previousPixel[1] == py || px >= canvasSize || py >= canvasSize || px < 0 || py < 0) {
            ...
            // color pixel
            // try {
                pixelBuffer[py][px].r += emitterColor[0] * 10 * contribution * attenuation;
                pixelBuffer[py][px].g += emitterColor[1] * 10 * contribution * attenuation;
                pixelBuffer[py][px].b += emitterColor[2] * 10 * contribution * attenuation;
            // } catch {
                // let y = 0;
            // }
        }
...
function emitPhotons() {
    ...
    let ex = pointLight.center[0] + Math.cos(rnd) * radius;
    let ey = pointLight.center[1] + Math.sin(rnd) * radius;

    let rayAngle = Math.random() * (Math.PI * 2);
    // let rayAngle = Math.random() * (Math.PI * 0.02) + 1.5;
